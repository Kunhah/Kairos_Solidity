...
// contract SwapFlat is ISwap, ReferralSystem {
...
//             referralRewards[referralAddress][swap.swapSteps[swap.swapSteps.length - 1].tokenOut] += distributedAmount;
...
//     function convertReferralRewardsToUSDT(address referral, address[] calldata tokens) external {
//         address usdt = PolygonChainlink.ADDRESS_USDT;
//         for (uint256 i = 0; i < tokens.length; i++) {
//             uint256 amount = referralRewards[referral][tokens[i]];
//             if (amount == 0) continue;
//             referralRewards[referral][tokens[i]] = 0;

//             if (tokens[i] != usdt) {
//                 (, uint256 amountOut) = SwapTokens.UniswapV3(tokens[i], usdt, amount, referral);
//             } else {
//                 IERC20(usdt).safeTransfer(referral, amount);
//             }
//         }
//     }
