...
contract SwapNoFee is ReferralSystem {
    ...
    */
    function _SwapTokens(address tokenIn, address tokenOut, uint256 amountIn, address receiver, PayTypes.DEX dex, bytes calldata extraData) internal returns(bool success, uint256 result) {
        ...
        else if (dex == PayTypes.DEX.QuickSwap) {
            return SwapTokens.Quickswap(tokenIn, tokenOut, amountIn, receiver);
        }
        else if (dex == PayTypes.DEX.Curve) {
            (int128 i, int128 j, address pool) = abi.decode(extraData, (int128, int128, address));
