...
contract Swap is ISwap, ReferralSystem {
    ...
    uint256 public constant FEE_BPS = 197_000; // 19.7%
    ...
    */
    function _Swap(PayTypes.Swap calldata swap, uint256 amount) internal returns(bool) {
        ...
        if (swap.swapSteps.length == 1) {
            ...
            if (swap.isFlat) {
                fee = (amount * FEE_FLAT) / BPS_DIVISOR;
            }
            else {
    ...
    function convertReferralRewardsToUSDT(address user, address[] calldata tokens) external {
        address usdt = PolygonChainlink.ADDRESS_USDT;
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 amount = distributeReferralRewards(ProtocolAddress, token, unclaimedFee[user][token]);
            if (amount == 0) continue;
            referralRewards[user][tokens[i]] = 0;

            if (tokens[i] != usdt) {
                (, uint256 amountOut) = SwapTokens.UniswapV3(tokens[i], usdt, amount, user);
            } else {
                IERC20(usdt).safeTransfer(referral, user);
            }
        }
    }
